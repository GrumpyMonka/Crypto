    ...
    ui(new Ui::FormEncrypt)
{
    ...
    qRegisterMetaType<RecordMap>("RecordMap");
...
void FormEncrypt::searchWord()
{
    ...
    for( int i = 0; i < THREAD_COUNT; ++i )
    {
        ...
        ThreadMonster* thread = new ThreadMonster( temp, list_word, this );
...
void FormEncrypt::slotOnThreadPrognoz( RecordMap record_map )
{
    wait_prognoz.count--;
    if ( record_map.first > wait_prognoz.record.first )
    {
        wait_prognoz.record = record_map;
    }
    if ( !wait_prognoz.count )
    {
        if ( wait_prognoz.record.first == -1 )
        {
            ui->statusbar->showMessage( "END" );
        }
        else
        {
            size_true_word = wait_prognoz.record.first;
            setDecryptKey( wait_prognoz.record.second );
            textOnEncrypt = SubLetterInText( startText, decrypt_key );
            ui->textEdit->setText( textOnEncrypt );
            resetViewKey();
            searchWord();
...
void FormEncrypt::WAIT( RecordList list )
{
    ...
    if ( !wait.count ){
        ...
        wait_prognoz.count = wait.list.size();
        wait_prognoz.record.first = -1;
        for( const auto& rec_list : wait.list )
        {
            ThreadPrognoz* thread = new ThreadPrognoz(
                [&]() -> RecordMap
                {
                    RecordMap result_rec;
                    result_rec.first = -1;
                    for ( const auto& word : rec_list.result )
                    {
                        ...
                        if ( !new_keys.empty() )
                        {
                            ...
                            if ( count >= result_rec.first && count >= size_true_word
                                 && new_keys != result_rec.second && -1 == used_decrypt_key.indexOf( new_keys ) )
                                result_rec = { count, new_keys };
                        }
                    }
                    return result_rec;
                }
            );
            connect( thread, SIGNAL( END( RecordMap ) ), this, SLOT( slotOnThreadPrognoz( RecordMap ) ) );
            thread->start();
        }
    }
}
